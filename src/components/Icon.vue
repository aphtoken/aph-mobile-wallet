<template>
  <div class="aph-icon" v-on="$listeners">
    <!-- logo -->
    <svg class="icon logo" v-if="name === 'logo'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      viewBox="0 0 324 406.9" style="enable-background:new 0 0 324 406.9;" xml:space="preserve">
      <g>
        <path class="fill st0" d="M35.6,371.6c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.6,0.4-0.9,0.4c-0.3,0-0.5-0.1-0.7-0.2c-0.2-0.2-0.4-0.4-0.5-0.6
          l-2.2-5.9H20.3l-2.2,5.9c-0.1,0.3-0.3,0.5-0.5,0.6c-0.2,0.2-0.5,0.2-0.7,0.2c-0.4,0-0.7-0.1-0.9-0.4c-0.2-0.3-0.4-0.6-0.4-1
          c0-0.1,0.1-0.4,0.3-1c0.2-0.6,0.4-1.3,0.8-2.2c0.3-0.9,0.7-1.9,1.1-3c0.4-1.1,0.8-2.2,1.2-3.3c0.4-1.2,0.9-2.3,1.3-3.4
          c0.4-1.1,0.8-2.2,1.2-3.1c0.4-1,0.7-1.8,0.9-2.5c0.3-0.7,0.4-1.2,0.6-1.5l1-2.8c0.1-0.2,0.2-0.3,0.2-0.5c0-0.1,0.1-0.2,0.1-0.3
          s0.1-0.2,0.1-0.3c0.1-0.2,0.3-0.4,0.4-0.5c0.2-0.2,0.4-0.3,0.8-0.3c0.3,0,0.6,0.1,0.8,0.3c0.2,0.2,0.3,0.4,0.4,0.5
          c0,0.1,0.1,0.2,0.1,0.3c0,0.1,0.1,0.2,0.1,0.3c0,0.2,0.1,0.3,0.2,0.5l1,2.8c0.1,0.3,0.3,0.8,0.5,1.5c0.3,0.7,0.6,1.5,1,2.5
          c0.4,1,0.8,2,1.2,3.1c0.4,1.1,0.9,2.3,1.3,3.4c0.4,1.1,0.8,2.2,1.2,3.3c0.4,1.1,0.8,2.1,1.1,3c0.3,0.9,0.6,1.6,0.8,2.2
          C35.5,371.2,35.6,371.5,35.6,371.6z M29.9,363.6L25.6,352l-4.3,11.6H29.9z"/>
        <path class="fill st0" d="M72.6,354.9c0,1.1-0.2,2.2-0.6,3.1c-0.4,1-1,1.8-1.7,2.5c-0.7,0.7-1.5,1.3-2.5,1.7c-0.9,0.4-1.9,0.6-3,0.6
          h-4.4v8.7c0,0.4-0.1,0.7-0.4,1c-0.3,0.3-0.6,0.4-0.9,0.4c-0.3,0-0.6-0.1-0.9-0.4c-0.3-0.3-0.4-0.6-0.4-1v-23.4
          c0-0.4,0.1-0.7,0.4-0.9c0.3-0.3,0.6-0.4,0.9-0.4h5.7c1.1,0,2.1,0.2,3,0.6c0.9,0.4,1.8,1,2.5,1.7c0.7,0.7,1.3,1.6,1.7,2.5
          C72.4,352.7,72.6,353.8,72.6,354.9z M70,354.9c0-0.7-0.1-1.4-0.4-2.1c-0.3-0.7-0.6-1.2-1.1-1.7c-0.5-0.5-1-0.9-1.6-1.2
          c-0.6-0.3-1.3-0.4-2-0.4h-4.3v10.8h4.3c0.7,0,1.4-0.1,2-0.4c0.6-0.3,1.2-0.7,1.6-1.2c0.5-0.5,0.8-1.1,1.1-1.7
          C69.9,356.3,70,355.6,70,354.9z"/>
        <path class="fill st0" d="M111.4,371.6c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.5,0.4-0.9,0.4c-0.3,0-0.6-0.1-0.9-0.4c-0.3-0.3-0.4-0.6-0.4-1
          v-10.4h-9.3v10.4c0,0.4-0.1,0.7-0.4,1c-0.3,0.3-0.6,0.4-0.9,0.4c-0.4,0-0.7-0.1-0.9-0.4c-0.2-0.3-0.4-0.6-0.4-1v-23.4
          c0-0.4,0.1-0.7,0.4-0.9c0.2-0.3,0.5-0.4,0.9-0.4c0.3,0,0.6,0.1,0.9,0.4c0.3,0.3,0.4,0.6,0.4,0.9v10.4h9.3v-10.4
          c0-0.4,0.1-0.7,0.4-0.9c0.3-0.3,0.6-0.4,0.9-0.4c0.4,0,0.7,0.1,0.9,0.4c0.2,0.3,0.4,0.6,0.4,0.9V371.6z"/>
        <path class="fill st0" d="M151.7,371.6c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.5,0.4-0.9,0.4h-12.9c-0.3,0-0.6-0.1-0.9-0.4
          c-0.2-0.3-0.4-0.6-0.4-1v-23.4c0-0.4,0.1-0.7,0.4-0.9c0.2-0.3,0.5-0.4,0.9-0.4h12.9c0.3,0,0.6,0.1,0.9,0.4c0.2,0.3,0.4,0.6,0.4,0.9
          c0,0.4-0.1,0.7-0.4,0.9c-0.2,0.3-0.5,0.4-0.9,0.4h-11.7v9.1h9.1c0.4,0,0.7,0.1,0.9,0.4c0.2,0.3,0.4,0.6,0.4,1
          c0,0.4-0.1,0.7-0.4,0.9s-0.5,0.4-0.9,0.4h-9.1v9.1h11.7c0.3,0,0.6,0.1,0.9,0.4C151.6,371,151.7,371.3,151.7,371.6z"/>
        <path class="fill st0" d="M192.3,371.6c0,0.4-0.1,0.7-0.4,1c-0.3,0.3-0.6,0.4-0.9,0.4h-12.9c-0.3,0-0.6-0.1-0.9-0.4
          c-0.3-0.3-0.4-0.6-0.4-1v-23.5c0-0.4,0.1-0.7,0.4-0.9c0.3-0.3,0.6-0.4,0.9-0.4c0.4,0,0.7,0.1,0.9,0.4c0.3,0.3,0.4,0.6,0.4,0.9v22.2
          H191c0.3,0,0.6,0.1,0.9,0.4C192.2,371,192.3,371.3,192.3,371.6z"/>
        <path class="fill st0" d="M229.6,371.6c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.5,0.4-0.9,0.4h-12.5c-0.4,0-0.7-0.1-0.9-0.4
          c-0.2-0.3-0.4-0.6-0.4-1c0-0.4,0.1-0.7,0.4-0.9s0.5-0.4,0.9-0.4h5v-20.8h-5c-0.4,0-0.7-0.1-0.9-0.4c-0.2-0.3-0.4-0.6-0.4-1
          c0-0.4,0.1-0.7,0.4-0.9c0.2-0.3,0.5-0.4,0.9-0.4h12.5c0.4,0,0.7,0.1,0.9,0.4c0.2,0.3,0.4,0.6,0.4,0.9c0,0.4-0.1,0.7-0.4,1
          c-0.2,0.3-0.5,0.4-0.9,0.4h-5v20.8h5c0.4,0,0.7,0.1,0.9,0.4C229.5,371,229.6,371.3,229.6,371.6z"/>
        <path class="fill st0" d="M268.8,365.4c0,1.1-0.2,2.1-0.6,3c-0.4,0.9-0.9,1.8-1.6,2.5c-0.7,0.7-1.5,1.3-2.4,1.7
          c-0.9,0.4-1.9,0.6-2.9,0.6c-1,0-2-0.2-2.9-0.6c-0.9-0.4-1.7-1-2.4-1.7c-0.7-0.7-1.2-1.5-1.6-2.5c-0.4-0.9-0.6-1.9-0.6-3v-10.8
          c0-1.1,0.2-2.1,0.6-3c0.4-0.9,0.9-1.8,1.6-2.5c0.7-0.7,1.5-1.3,2.4-1.7c0.9-0.4,1.9-0.6,2.9-0.6c1,0,2,0.2,2.9,0.6
          c0.9,0.4,1.7,1,2.4,1.7c0.7,0.7,1.2,1.5,1.6,2.5c0.4,0.9,0.6,1.9,0.6,3V365.4z M266.2,365.4v-10.8c0-0.7-0.1-1.3-0.4-1.9
          c-0.3-0.6-0.6-1.2-1-1.6c-0.4-0.5-0.9-0.8-1.5-1.1c-0.6-0.3-1.2-0.4-1.9-0.4c-0.7,0-1.3,0.1-1.9,0.4c-0.6,0.3-1.1,0.6-1.5,1.1
          c-0.4,0.5-0.8,1-1,1.6c-0.3,0.6-0.4,1.3-0.4,1.9v10.8c0,0.7,0.1,1.3,0.4,1.9c0.3,0.6,0.6,1.2,1,1.6c0.4,0.5,0.9,0.8,1.5,1.1
          c0.6,0.3,1.2,0.4,1.9,0.4c0.7,0,1.3-0.1,1.9-0.4c0.6-0.3,1.1-0.6,1.5-1.1c0.4-0.5,0.8-1,1-1.6C266.1,366.7,266.2,366,266.2,365.4z"
          />
        <path class="fill st0" d="M308.4,371.6c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.5,0.4-0.9,0.4c-0.5,0-0.9-0.2-1.1-0.7l-10.5-19.1v18.4
          c0,0.4-0.1,0.7-0.4,1c-0.2,0.3-0.5,0.4-0.9,0.4c-0.4,0-0.7-0.1-0.9-0.4c-0.2-0.3-0.4-0.6-0.4-1v-23.5c0-0.4,0.1-0.7,0.4-0.9
          c0.2-0.3,0.5-0.4,0.9-0.4c0.5,0,0.9,0.2,1.1,0.7l10.5,19.1v-18.5c0-0.4,0.1-0.7,0.4-0.9c0.2-0.3,0.5-0.4,0.9-0.4
          c0.4,0,0.7,0.1,0.9,0.4c0.2,0.3,0.4,0.6,0.4,0.9V371.6z"/>
      </g>
      <path class="fill st1" d="M162,34C89.8,34,31,92.8,31,165s58.8,131,131,131s131-58.8,131-131S234.2,34,162,34z M267.6,165
        c0,14.3-2.9,28.5-8.6,41.7L188.4,62.7C233.9,74.4,267.6,115.9,267.6,165z M161.1,128.2l25.6,52.1h-51.1L161.1,128.2z M133.6,63.3
        L69.1,195.1l-4.6,10.4c-5.3-12.8-8.1-26.6-8.1-40.5C56.4,116.6,89.1,75.7,133.6,63.3L133.6,63.3z M162,270.6
        c-32,0-62.3-14.5-82.3-39.5h103.9l13.7-29l14.2,29h32.6C224.2,256.1,194,270.6,162,270.6z"/>
    </svg>

    <!-- logo-mark -->
    <svg class="icon logo-mark" v-if="name === 'logo-mark'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      viewBox="0 0 324 324" style="enable-background:new 0 0 324 324;" xml:space="preserve">
      <path class="fill st0" d="M162,34C89.8,34,31,92.8,31,165s58.8,131,131,131s131-58.8,131-131S234.2,34,162,34z M267.6,165
        c0,14.3-2.9,28.5-8.6,41.7L188.4,62.7C233.9,74.4,267.6,115.9,267.6,165z M161.1,128.2l25.6,52.1h-51.1L161.1,128.2z M133.6,63.3
        L69.1,195.1l-4.6,10.4c-5.3-12.8-8.1-26.6-8.1-40.5C56.4,116.6,89.1,75.7,133.6,63.3L133.6,63.3z M162,270.6
        c-32,0-62.3-14.5-82.3-39.5h103.9l13.7-29l14.2,29h32.6C224.2,256.1,194,270.6,162,270.6z"/>
    </svg>

    <!-- word-mark -->
    <svg class="icon word-mark" v-if="name === 'word-mark'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 515.85 46.44">
      <path class="fill cls-1" d="M35.49,43.13a2.36,2.36,0,0,1-.66,1.69,2.17,2.17,0,0,1-1.66.68,2.25,2.25,0,0,1-1.28-.4,2.14,2.14,0,0,1-.84-1.1L27.17,33.55H8.51L4.63,44a2.14,2.14,0,0,1-.84,1.1,2.25,2.25,0,0,1-1.28.4,2.19,2.19,0,0,1-1.66-.68,2.36,2.36,0,0,1-.66-1.69,11.72,11.72,0,0,1,.5-1.82C1,40.27,1.47,39,2,37.4S3.24,34.08,4,32.14s1.43-3.91,2.19-5.91,1.51-4.08,2.28-6.11l2.13-5.57q1-2.54,1.66-4.41c.46-1.25.79-2.13,1-2.63L15,2.63a2.33,2.33,0,0,0,.31-.88c.09-.2.16-.39.22-.56s.14-.33.22-.5a5.72,5.72,0,0,1,.78-1,1.86,1.86,0,0,1,1.35-.47,1.86,1.86,0,0,1,1.35.47,5.72,5.72,0,0,1,.78,1c.08.17.15.34.22.5s.13.36.22.56a2.33,2.33,0,0,0,.31.88l1.75,4.88c.17.5.49,1.38,1,2.63l1.69,4.41,2.13,5.57c.77,2,1.53,4.06,2.28,6.11s1.49,4,2.2,5.91,1.34,3.69,1.9,5.26,1,2.87,1.35,3.91A11.72,11.72,0,0,1,35.49,43.13ZM25.41,28.86,17.84,8.33,10.27,28.86Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M100.71,13.46A14.46,14.46,0,0,1,99.64,19a13.82,13.82,0,0,1-2.94,4.5,14.27,14.27,0,0,1-4.35,3,12.63,12.63,0,0,1-5.29,1.13h-7.7V43.13a2.31,2.31,0,0,1-.69,1.69A2.27,2.27,0,0,1,77,45.5a2.21,2.21,0,0,1-1.59-.68,2.28,2.28,0,0,1-.7-1.69V1.63A2.32,2.32,0,0,1,75.42,0,2.13,2.13,0,0,1,77-.75H87A12.67,12.67,0,0,1,92.33.38a14.46,14.46,0,0,1,4.36,3,13.83,13.83,0,0,1,3,4.51A14.46,14.46,0,0,1,100.71,13.46Zm-4.63,0a9.55,9.55,0,0,0-.72-3.69,9.94,9.94,0,0,0-1.93-3,9.35,9.35,0,0,0-2.87-2A8.47,8.47,0,0,0,87,4H79.36V23H87a8.47,8.47,0,0,0,3.53-.75,9.35,9.35,0,0,0,2.87-2,9.87,9.87,0,0,0,1.93-3A9.47,9.47,0,0,0,96.08,13.46Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M169,43.13a2.36,2.36,0,0,1-.66,1.69,2.1,2.1,0,0,1-1.59.68,2.15,2.15,0,0,1-1.57-.68,2.3,2.3,0,0,1-.68-1.69V24.72h-16.4V43.13a2.31,2.31,0,0,1-.69,1.69,2.15,2.15,0,0,1-1.57.68,2.11,2.11,0,0,1-1.59-.68,2.36,2.36,0,0,1-.66-1.69V1.63A2.38,2.38,0,0,1,144.24,0a2.08,2.08,0,0,1,1.59-.72A2.12,2.12,0,0,1,147.4,0a2.32,2.32,0,0,1,.69,1.66V20h16.4V1.63A2.31,2.31,0,0,1,165.17,0a2.12,2.12,0,0,1,1.57-.72,2.07,2.07,0,0,1,1.59.72A2.38,2.38,0,0,1,169,1.63Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M240.09,43.13a2.36,2.36,0,0,1-.66,1.69,2,2,0,0,1-1.53.68H215a2,2,0,0,1-1.53-.68,2.36,2.36,0,0,1-.66-1.69V1.63A2.38,2.38,0,0,1,213.46,0,2,2,0,0,1,215-.75H237.9a2,2,0,0,1,1.53.71,2.35,2.35,0,0,1,.66,1.64,2.32,2.32,0,0,1-.66,1.67A2.05,2.05,0,0,1,237.9,4H217.25V20h16a2.09,2.09,0,0,1,1.59.69,2.35,2.35,0,0,1,.66,1.69,2.29,2.29,0,0,1-.66,1.63,2.08,2.08,0,0,1-1.59.68h-16V40.81H237.9a2,2,0,0,1,1.53.69A2.27,2.27,0,0,1,240.09,43.13Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M311.57,43.13a2.31,2.31,0,0,1-.69,1.69,2.14,2.14,0,0,1-1.56.68H286.5a2.21,2.21,0,0,1-1.59-.68,2.31,2.31,0,0,1-.69-1.69V1.57a2.22,2.22,0,0,1,.69-1.63,2.22,2.22,0,0,1,1.59-.69,2.27,2.27,0,0,1,1.65.69,2.2,2.2,0,0,1,.7,1.63V40.81h20.47a2.15,2.15,0,0,1,1.56.69A2.22,2.22,0,0,1,311.57,43.13Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M377.29,43.13a2.36,2.36,0,0,1-.66,1.69,2.12,2.12,0,0,1-1.6.68H352.88a2.12,2.12,0,0,1-1.6-.68,2.36,2.36,0,0,1-.66-1.69,2.27,2.27,0,0,1,.66-1.63,2.13,2.13,0,0,1,1.6-.69h8.82V4h-8.82a2.13,2.13,0,0,1-1.6-.69,2.36,2.36,0,0,1-.66-1.69,2.27,2.27,0,0,1,.66-1.63,2.13,2.13,0,0,1,1.6-.69H375a2.13,2.13,0,0,1,1.6.69,2.27,2.27,0,0,1,.66,1.63,2.36,2.36,0,0,1-.66,1.69A2.13,2.13,0,0,1,375,4h-8.82V40.81H375a2.13,2.13,0,0,1,1.6.69A2.27,2.27,0,0,1,377.29,43.13Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M446.38,32.05a13.56,13.56,0,0,1-1,5.29,14.14,14.14,0,0,1-2.83,4.35,13,13,0,0,1-4.2,2.94,12.87,12.87,0,0,1-10.23,0,13.06,13.06,0,0,1-4.2-2.94,14.3,14.3,0,0,1-2.82-4.35,13.56,13.56,0,0,1-1-5.29V12.9a13.56,13.56,0,0,1,1-5.29,14.18,14.18,0,0,1,2.82-4.35,13.06,13.06,0,0,1,4.2-2.94,12.76,12.76,0,0,1,10.23,0,13,13,0,0,1,4.2,2.94,14,14,0,0,1,2.83,4.35,13.56,13.56,0,0,1,1,5.29Zm-4.63,0V12.85a9,9,0,0,0-.68-3.45,9.38,9.38,0,0,0-1.84-2.85,8.24,8.24,0,0,0-12,0,9.38,9.38,0,0,0-1.84,2.85,8.77,8.77,0,0,0-.68,3.45V32a8.74,8.74,0,0,0,.68,3.45,9.38,9.38,0,0,0,1.84,2.85,8.65,8.65,0,0,0,2.71,2,7.95,7.95,0,0,0,6.61,0,8.69,8.69,0,0,0,2.72-2,9.38,9.38,0,0,0,1.84-2.85A8.92,8.92,0,0,0,441.75,32Z"
        transform="translate(-0.19 0.75)" />
      <path class="fill cls-1" d="M516.05,43.13a2.36,2.36,0,0,1-.66,1.69,2.12,2.12,0,0,1-1.6.68,2,2,0,0,1-1.94-1.18l-18.59-33.8V43.13a2.36,2.36,0,0,1-.66,1.69,2.1,2.1,0,0,1-1.59.68,2.12,2.12,0,0,1-1.6-.68,2.36,2.36,0,0,1-.65-1.69V1.57a2.26,2.26,0,0,1,.65-1.63A2.13,2.13,0,0,1,491-.75,2.07,2.07,0,0,1,493,.44l18.59,33.8V1.57a2.27,2.27,0,0,1,.66-1.63,2.11,2.11,0,0,1,1.59-.69,2.13,2.13,0,0,1,1.6.69,2.27,2.27,0,0,1,.66,1.63Z"
        transform="translate(-0.19 0.75)" />
    </svg>

    <!-- create -->
    <svg class="icon create" v-if="name === 'create'" viewBox="0 0 22 24" xmlns="http://www.w3.org/2000/svg">
      <g class="fill">
        <path d="m10.5 7.6v3.9h-3.9a.5.5 0 0 0 0 1h3.9v3.9a.5.5 0 0 0 1 0v-3.9h3.9a.5.5 0 0 0 0-1h-3.9v-3.9a.5.5 0 0 0 -1 0z" />
        <path d="m21.73 5.56-10.5-5.5a.47.47 0 0 0 -.46 0l-10.5 5.5a.48.48 0 0 0 -.27.44v12a.48.48 0 0 0 .27.44l10.5 5.5a.47.47 0 0 0 .46 0l10.5-5.5a.48.48 0 0 0 .27-.44v-12a.48.48 0 0 0 -.27-.44zm-.73 12.14-10 5.24-10-5.24v-11.4l10-5.24 10 5.24z"
        />
      </g>
    </svg>

    <!-- wallet -->
    <svg class="icon wallet" v-if="name === 'wallet'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 20">
      <path class="fill cls-1" d="M23,12H21.5V4.5A.5.5,0,0,0,21,4H2a.47.47,0,0,0-.35.15l-2,2a.36.36,0,0,0-.11.16.41.41,0,0,0,0,.19v17A.5.5,0,0,0,0,24H19a.5.5,0,0,0,.5-.5V22H21a.5.5,0,0,0,.5-.5V18H23a.5.5,0,0,0,.5-.5v-5A.5.5,0,0,0,23,12ZM2.21,5H20.5v7h-1V6.5A.5.5,0,0,0,19,6H1.21ZM18.5,23H.5V7h18v5H13a.47.47,0,0,0-.35.15l-2.5,2.5a.48.48,0,0,0,0,.7l2.5,2.5A.47.47,0,0,0,13,18h5.5Zm2-2h-1V18h1Zm2-4H13.21l-2-2,2-2H22.5Z"
        transform="translate(0.5 -4)" />
      <path class="fill cls-1" d="M14,16a.5.5,0,0,0,.5-.5v-1a.5.5,0,0,0-1,0v1A.5.5,0,0,0,14,16Z" transform="translate(0.5 -4)" />
    </svg>

    <!-- arrow-left -->
    <svg class="icon arrow-left" v-if="name === 'arrow-left'"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 19.11 13.54">
      <path class="fill back--path" d="M8.7,3.44a.7.7,0,0,1,1,0,.72.72,0,0,1,0,1L4.83,9.32h16a.67.67,0,0,1,.68.66h0a.69.69,0,0,1-.67.69H4.79l4.87,4.87a.69.69,0,0,1,0,1l0,0a.68.68,0,0,1-.95.05l-.05-.05-6-6a.69.69,0,0,1,0-1l0,0Z" transform="translate(-2.44 -3.23)"/>
    </svg>

    <!-- arrow-right -->
    <svg class="icon arrow-right" v-if="name === 'arrow-right'"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 19.11 13.54">
      <path class="fill back--path" d="M8.7,3.44a.7.7,0,0,1,1,0,.72.72,0,0,1,0,1L4.83,9.32h16a.67.67,0,0,1,.68.66h0a.69.69,0,0,1-.67.69H4.79l4.87,4.87a.69.69,0,0,1,0,1l0,0a.68.68,0,0,1-.95.05l-.05-.05-6-6a.69.69,0,0,1,0-1l0,0Z" transform="translate(-2.44 -3.23)"/>
    </svg>

    <!-- arrow-down -->
    <svg class="icon arrow-down" v-if="name === 'arrow-down'"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 13.54 19.11">
      <path class="fill back--path" d="M8.7,3.44a.7.7,0,0,1,1,0,.72.72,0,0,1,0,1L4.83,9.32h16a.67.67,0,0,1,.68.66h0a.69.69,0,0,1-.67.69H4.79l4.87,4.87a.69.69,0,0,1,0,1l0,0a.68.68,0,0,1-.95.05l-.05-.05-6-6a.69.69,0,0,1,0-1l0,0Z" transform="translate(-2.44 -3.23)"/>
    </svg>

    <!-- chevron-down -->
    <svg class="icon chevron-down" v-if="name === 'chevron-down'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 9 5">
      <path class="fill cls-1" d="M25,27.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7L25,26.29l3.65-3.64a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,25,27.5Z"
        transform="translate(-20.5 -22.5)" />
    </svg>

    <!-- chevron-up -->
    <svg class="icon chevron-up" v-if="name === 'chevron-up'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 9 5">
      <path class="fill cls-1" d="M25,27.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7L25,26.29l3.65-3.64a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,25,27.5Z"
        transform="translate(-20.5 -22.5)" />
    </svg>

    <!-- chevron-left -->
    <svg class="icon chevron-left" v-if="name === 'chevron-left'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5 9">
      <path class="fill cls-1" d="M25,27.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7L25,26.29l3.65-3.64a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,25,27.5Z"
        transform="translate(-20.5 -22.5)" />
    </svg>

    <!-- chevron-right -->
    <svg class="icon chevron-right" v-if="name === 'chevron-right'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 5 9">
      <path class="fill cls-1" d="M25,27.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7L25,26.29l3.65-3.64a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,25,27.5Z"
        transform="translate(-20.5 -22.5)" />
    </svg>

    <!-- add -->
    <svg class="icon add" v-if="name === 'add'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 7 7">
      <path class="fill add--path" d="M25,28.5a.5.5,0,0,1-.5-.5V22a.5.5,0,0,1,1,0v6A.5.5,0,0,1,25,28.5Z" transform="translate(-21.5 -21.5)"/>
      <path class="fill add--path" d="M28,25.5H22a.5.5,0,0,1,0-1h6a.5.5,0,0,1,0,1Z" transform="translate(-21.5 -21.5)" />
    </svg>

    <!-- remove -->
    <svg class="icon remove" v-if="name === 'remove'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 7 7">
      <path class="fill remove--path" d="M25,28.5a.5.5,0,0,1-.5-.5V22a.5.5,0,0,1,1,0v6A.5.5,0,0,1,25,28.5Z" />
      <path class="fill remove--path" d="M28,25.5H22a.5.5,0,0,1,0-1h6a.5.5,0,0,1,0,1Z" />
    </svg>

    <!-- copy -->
    <svg class="icon copy" v-if="name === 'copy'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16.49 13">
      <path class="fill st0" d="M23.25,31.5a.5.5,0,0,1-.35-.15l-6-6a.5.5,0,0,1,0-.7l6-6a.5.5,0,0,1,.71,0l6,6a.5.5,0,0,1,0,.7l-6,6A.51.51,0,0,1,23.25,31.5ZM18,25l5.29,5.29L28.55,25l-5.3-5.29Z" transform="translate(-16.75 -18.5)" />
      <path class="fill st1" d="M27.1,31.49a.45.45,0,0,1-.36-.13l-6.31-5.67a.5.5,0,0,1,0-.71l5.68-6.31a.49.49,0,0,1,.7,0l6.31,5.67a.5.5,0,0,1,0,.71l-5.68,6.31A.52.52,0,0,1,27.1,31.49Zm-5.63-6.21,5.56,5,5-5.57-5.56-5Z" transform="translate(-16.75 -18.5)" />
    </svg>

    <!-- dashboard -->
    <svg class="icon dashboard" v-if="name === 'dashboard'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M9.4,10.73l1.5,1.5a.48.48,0,0,0,.7,0l1.5-1.5a.5.5,0,0,0,0-.71l-1.5-1.5a.5.5,0,0,0-.7,0L9.4,10A.5.5,0,0,0,9.4,10.73Zm1.85-1.15.79.8-.79.79-.79-.79Z"
        transform="translate(-0.25 1.63)" />
      <path class="fill cls-1" d="M22,3.93l-10.5-5.5a.53.53,0,0,0-.46,0L.52,3.93a.5.5,0,0,0-.27.45v12a.5.5,0,0,0,.27.44L11,22.32a.47.47,0,0,0,.46,0L22,16.82a.5.5,0,0,0,.27-.44v-12A.5.5,0,0,0,22,3.93Zm-.73,12.14-10,5.24-10-5.24V4.68l10-5.24,10,5.24Z"
        transform="translate(-0.25 1.63)" />
      <path class="fill cls-1" d="M8.92,5.5l-5,4.5a.5.5,0,0,0-.17.38.5.5,0,0,0,.17.37l5,4.5a.51.51,0,0,0,.33.13h4a.51.51,0,0,0,.33-.13l5-4.5a.5.5,0,0,0,.17-.37.5.5,0,0,0-.17-.38l-5-4.5a.5.5,0,0,0-.33-.12h-4A.5.5,0,0,0,8.92,5.5Zm4.14.88,4.44,4-4.44,4H9.44L5,10.38l4.44-4Z"
        transform="translate(-0.25 1.63)" />
    </svg>

    <!-- history -->
    <svg class="icon history" v-if="name === 'history'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 24">
      <path class="fill cls-1" d="M19,7V2.5a.51.51,0,0,0-.22-.42l-3-2A.54.54,0,0,0,15.5,0h-7a.54.54,0,0,0-.28.08l-3,2A.51.51,0,0,0,5,2.5V7a.49.49,0,0,0,.2.4l6,4.6-6,4.6A.49.49,0,0,0,5,17v4.5a.51.51,0,0,0,.22.42l3,2A.54.54,0,0,0,8.5,24h7a.54.54,0,0,0,.28-.08l3-2A.51.51,0,0,0,19,21.5V17a.49.49,0,0,0-.2-.4l-6-4.6,6-4.6A.49.49,0,0,0,19,7ZM8.65,1h6.7L17.6,2.5,15.35,4H8.65L6.4,2.5ZM18,21.23,15.35,23H8.65L6,21.23v-4l6-4.62,6,4.62Zm-6-9.86L6,6.75V3.43L8.22,4.92A.54.54,0,0,0,8.5,5h7a.54.54,0,0,0,.28-.08L18,3.43V6.75Z"
        transform="translate(-5)" />
      <path class="fill cls-1" d="M14.19,6.61,12,8.36,9.81,6.61a.5.5,0,0,0-.62.78l2.5,2a.49.49,0,0,0,.62,0l2.5-2a.5.5,0,1,0-.62-.78Z"
        transform="translate(-5)" />
      <path class="fill cls-1" d="M11.5,15v1a.5.5,0,0,0,1,0V15a.5.5,0,0,0-1,0Z" transform="translate(-5)" />
      <path class="fill cls-1" d="M12.5,18a.5.5,0,0,0-1,0v.69L8.88,20H7.5a.5.5,0,0,0,0,1H9A.54.54,0,0,0,9.22,21L12,19.56,14.78,21A.54.54,0,0,0,15,21h1.5a.5.5,0,0,0,0-1H15.12L12.5,18.69Z"
        transform="translate(-5)" />
    </svg>

    <!-- settings -->
    <svg class="icon settings" v-if="name === 'settings'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 20">
      <path class="fill cls-1" d="M76,15.06A.28.28,0,0,0,76,15v0a.61.61,0,0,0,0-.12s0,0,0-.06l-5.5-9.5A.5.5,0,0,0,70,5H58a.46.46,0,0,0-.23.06h0l0,0a.57.57,0,0,0-.13.14h0l-5.5,9.5a.5.5,0,0,0,0,.5l5.5,9.5h0a.4.4,0,0,0,.14.15s0,0,0,0A.51.51,0,0,0,58,25H70a.48.48,0,0,0,.43-.25l5.5-9.5a.06.06,0,0,0,0,0A.28.28,0,0,0,76,15.06Zm-18.4.55.85-9.34,8.49,4.25Zm10-4.29L68,21.66l-9.9-5.17Zm-.54,11-8.56,1.63-.82-6.53ZM70.08,6.64l4,6.94-5.48-3.29ZM67.74,9.81,60.12,6h9.14Zm-11.18,6-3.32-1.11,4.06-7Zm0,1,.66,5.24-3.61-6.23Zm11.75,6.17L69,24H63.3Zm.25-11.64,6.17,3.7L69,21.28ZM70,23.59l-.8-1.06,3.71-4Z"
        transform="translate(-52 -5)" />
    </svg>

    <!-- send -->
    <svg class="icon send" v-if="name === 'send'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M13.59,6.94a.5.5,0,0,0-.38,0,.36.36,0,0,0-.16.11l-4,4a.49.49,0,0,0,.7.7L12.9,8.61V18.4a.5.5,0,0,0,1,0V8.61l3.15,3.14a.48.48,0,0,0,.7,0,.48.48,0,0,0,0-.7l-4-4A.36.36,0,0,0,13.59,6.94Z"
        transform="translate(-2.4 -0.9)" />
      <path class="fill cls-1" d="M24.13,6.46,13.63,1a.47.47,0,0,0-.46,0L2.67,6.46a.48.48,0,0,0-.27.44v12a.48.48,0,0,0,.27.44l10.5,5.5a.47.47,0,0,0,.46,0l10.5-5.5a.48.48,0,0,0,.27-.44V6.9A.48.48,0,0,0,24.13,6.46ZM23.4,18.6l-10,5.24L3.4,18.6V7.2L13.4,2l10,5.24Z"
        transform="translate(-2.4 -0.9)" />
    </svg>

    <!-- receive -->
    <svg class="icon receive" v-if="name === 'receive'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 23.96">
      <path class="fill cls-1" d="M10.81,18a.5.5,0,0,0,.38,0,.36.36,0,0,0,.16-.11l4-4a.49.49,0,1,0-.7-.7L11.5,16.33V6.54a.5.5,0,1,0-1,0v9.79L7.35,13.19a.49.49,0,0,0-.68,0l0,0a.49.49,0,0,0,0,.68l0,0,4,4A.36.36,0,0,0,10.81,18Z"
        transform="translate(0 -0.04)" />
      <path class="fill cls-1" d="M.27,18.48l10.5,5.46a.47.47,0,0,0,.46,0l10.5-5.46A.48.48,0,0,0,22,18V6a.48.48,0,0,0-.27-.44L11.23.1a.47.47,0,0,0-.46,0L.27,5.6A.48.48,0,0,0,0,6V18A.48.48,0,0,0,.27,18.48ZM1,6.34,11,1.1,21,6.34v11.4l-10,5.2L1,17.7Z"
        transform="translate(0 -0.04)" />
    </svg>

    <!-- reset -->
    <svg class="icon reset" v-if="name === 'reset'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8.25 9">
      <path class="fill cls-1" d="M8.25,2H8.3a.47.47,0,0,1,.25.07l3.57,2a.51.51,0,0,1,.26.43v4a.49.49,0,0,1-.27.44L8.49,11A.64.64,0,0,1,8,11l-3.57-2a.51.51,0,0,1-.26-.44v-4A.52.52,0,0,1,4.39,4L8,2.07A.53.53,0,0,1,8.25,2Zm3.13,2.83L8.27,3.07,5.12,4.77V8.18L8.23,10l3.15-1.69Z"
        transform="translate(-4.13 -2.01)" />
    </svg>

    <!-- plus -->
    <svg class="icon plus" v-if="name === 'plus'" xxmlns="http://www.w3.org/2000/svg" viewBox="0 0 7 7">
      <path class="fill cls-1" d="M25,28.5a.5.5,0,0,1-.5-.5V22a.5.5,0,0,1,1,0v6A.5.5,0,0,1,25,28.5Z" transform="translate(-21.5 -21.5)"
      />
      <path class="fill cls-1" d="M28,25.5H22a.5.5,0,0,1,0-1h6a.5.5,0,0,1,0,1Z" transform="translate(-21.5 -21.5)" />
    </svg>

    <!-- minus -->
    <svg class="icon minus" v-if="name === 'minus'" xxmlns="http://www.w3.org/2000/svg" viewBox="0 0 7 7">
      <path class="fill cls-1" d="M28,25.5H22a.5.5,0,0,1,0-1h6a.5.5,0,0,1,0,1Z" transform="translate(-21.5 -21.5)" />
    </svg>

    <!-- confirmed-big -->
    <svg class="icon confirmed-big" v-if="name === 'confirmed-big'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.26 24.24">
      <path class="fill cls-1" d="M11.13,24.22a.6.6,0,0,1-.29-.08L.34,18.64A.61.61,0,0,1,0,18.09v-12a.61.61,0,0,1,.34-.55L10.84.07a.6.6,0,0,1,.58,0l10.5,5.5a.61.61,0,0,1,.34.55v12a.61.61,0,0,1-.34.55l-10.5,5.55A.71.71,0,0,1,11.13,24.22ZM1.25,17.74l9.88,5.18L21,17.74V6.5L11.13,1.33,1.25,6.5Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M9.63,16.75h0a.6.6,0,0,1-.44-.19l-4-4a.62.62,0,0,1,.88-.88h0l3.56,3.54,7.56-7.56a.62.62,0,1,1,.88.88h0l-8,8A.62.62,0,0,1,9.63,16.75Z"
        transform="translate(0 0)" />
    </svg>

    <!-- confirmed -->
    <svg class="icon confirmed" v-if="name === 'confirmed'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.51 24.49">
      <path class="fill cls-1" d="M22.51,10,12,4.54a.73.73,0,0,0-.7,0L.82,10a.73.73,0,0,0-.41.67v12a.77.77,0,0,0,.4.67l10.5,5.49a.67.67,0,0,0,.34.1l.15,0,.21,0,10.5-5.55a.74.74,0,0,0,.41-.66v-12A.75.75,0,0,0,22.51,10Zm-1.09,1.12V22.24l-9.76,5.11L1.91,22.24V11.15l9.75-5.1Z"
        transform="translate(-0.41 -4.45)" />
      <path class="fill cls-1" d="M18.19,12.15a.75.75,0,0,0-.53-.22h0a.75.75,0,0,0-.53.22L9.66,19.62,6.19,16.17A.75.75,0,0,0,5.66,16h0a.75.75,0,0,0-.75.75.77.77,0,0,0,.22.53l4,4a.72.72,0,0,0,.53.23h0a.73.73,0,0,0,.52-.24l8-8a.77.77,0,0,0,.22-.53A.75.75,0,0,0,18.19,12.15Z"
        transform="translate(-0.41 -4.45)" />
    </svg>

    <!-- calendar -->
    <svg class="icon calendar" v-if="name === 'calendar'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.75 18.25">
      <path class="fill cls-1" d="M25.1,5.12H20V5a.88.88,0,1,0-1.75,0v.12H14V5a.88.88,0,1,0-1.75,0v.12H8V5A.88.88,0,1,0,6.23,5v.12H2.1A.84.84,0,0,0,1.23,6V21.5a.83.83,0,0,0,.87.87h23A.83.83,0,0,0,26,21.5V6A.84.84,0,0,0,25.1,5.12ZM7.1,8.87A.84.84,0,0,0,8,8V6.87h4.25V8A.88.88,0,0,0,14,8V6.87h4.24V8A.88.88,0,0,0,20,8V6.87h4.25V9.12H3V6.87H6.23V8A.83.83,0,0,0,7.1,8.87ZM3,20.62V10.87H24.22v9.75Z"
        transform="translate(-1.23 -4.12)" />
      <path class="fill cls-1" d="M10.9,13.12h-2a.88.88,0,1,0,0,1.75h2a.88.88,0,0,0,0-1.75Z" transform="translate(-1.23 -4.12)" />
      <path class="fill cls-1" d="M18.4,13.12h-5a.88.88,0,1,0,0,1.75h5a.88.88,0,1,0,0-1.75Z" transform="translate(-1.23 -4.12)" />
      <path class="fill cls-1" d="M10.9,16.12h-2a.88.88,0,1,0,0,1.75h2a.88.88,0,0,0,0-1.75Z" transform="translate(-1.23 -4.12)" />
      <path class="fill cls-1" d="M18.4,16.12h-5a.88.88,0,1,0,0,1.75h5a.88.88,0,1,0,0-1.75Z" transform="translate(-1.23 -4.12)" />
    </svg>

    <!-- cancel -->
    <svg class="icon cancel" v-if="name === 'cancel'" viewBox="0 0 22 24" xmlns="http://www.w3.org/2000/svg">
      <g class="fill">
        <path class="x" d="m10.5 7.6v3.9h-3.9a.5.5 0 0 0 0 1h3.9v3.9a.5.5 0 0 0 1 0v-3.9h3.9a.5.5 0 0 0 0-1h-3.9v-3.9a.5.5 0 0 0 -1 0z" />
        <path d="m21.73 5.56-10.5-5.5a.47.47 0 0 0 -.46 0l-10.5 5.5a.48.48 0 0 0 -.27.44v12a.48.48 0 0 0 .27.44l10.5 5.5a.47.47 0 0 0 .46 0l10.5-5.5a.48.48 0 0 0 .27-.44v-12a.48.48 0 0 0 -.27-.44zm-.73 12.14-10 5.24-10-5.24v-11.4l10-5.24 10 5.24z" />
      </g>
    </svg>

    <!-- search -->
    <svg class="icon search" v-if="name === 'search'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8.99 10.89">
      <path class="fill cls-1" d="M17,7.46,15,4.15a.48.48,0,0,0-.43-.25H10.52a.5.5,0,0,0-.44.26L8.13,7.73a.5.5,0,0,0,0,.5l2,3.31a.5.5,0,0,0,.43.25h3.78L16,14.55a.47.47,0,0,0,.42.24.49.49,0,0,0,.43-.75l-1.64-2.77L17,8A.5.5,0,0,0,17,7.46ZM14.3,10.78H10.82L9.14,8,10.81,4.9H14.3L16,7.72Z"
        transform="translate(-8.06 -3.9)" />
    </svg>

    <!-- eye-open -->
    <svg class="icon eye-open" v-if="name === 'eye-open'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.97 10.62">
      <path class="fill cls-1" d="M8.32,3.34a.5.5,0,0,0-.67,0l-1.6,1.6a.5.5,0,0,0,0,.71h0l1.6,1.61A.46.46,0,0,0,8,7.4a.41.41,0,0,0,.31-.14L9.93,5.65a.5.5,0,0,0,.14-.35.54.54,0,0,0-.14-.36ZM8,6.19l-.9-.89L8,4.4l.89.9Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M15.81,4.94,10.46.12A.5.5,0,0,0,10.13,0H5.85a.54.54,0,0,0-.34.12L.17,4.94a.49.49,0,0,0,0,.74l5.34,4.81a.55.55,0,0,0,.34.13h4.28a.51.51,0,0,0,.33-.13l5.35-4.81a.51.51,0,0,0,0-.74ZM9.93,9.62H6L1.3,5.31,6,1H9.93l4.8,4.31Z"
        transform="translate(0 0)" />
    </svg>

    <!-- eye-closed -->
    <svg class="icon eye-closed" v-if="name === 'eye-closed'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.97 10.62">
      <path class="fill cls-1" d="M7.65,3.34l-1.6,1.6a.5.5,0,0,0,0,.71h0l1.6,1.61A.46.46,0,0,0,8,7.4a.41.41,0,0,0,.31-.14L9.93,5.65a.5.5,0,0,0,.14-.35.54.54,0,0,0-.14-.36L8.32,3.34A.5.5,0,0,0,7.65,3.34ZM8,6.19l-.9-.89L8,4.4l.89.9Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M15.81,4.94,10.46.12A.5.5,0,0,0,10.13,0H5.85a.54.54,0,0,0-.34.12L.17,4.94a.49.49,0,0,0,0,.74l5.34,4.81a.55.55,0,0,0,.34.13h4.28a.51.51,0,0,0,.33-.13l5.35-4.81a.51.51,0,0,0,0-.74ZM9.93,9.62H6L1.3,5.31,6,1H9.93l4.8,4.31Z"
        transform="translate(0 0)" />
      <path class="fill ls-1" d="M3.28,10.51a.52.52,0,0,1-.35-.14.5.5,0,0,1,0-.71L12.33.25a.51.51,0,0,1,.71,0A.5.5,0,0,1,13,1l-9.4,9.41A.54.54,0,0,1,3.28,10.51Z"
        transform="translate(0 0)" />
    </svg>

    <!-- user -->
    <svg clas s="icon user" v-if="name === 'user'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21.5 24.4">
      <path class="fill cls-1" d="M23.94,6.36,13.64.76a.51.51,0,0,0-.48,0L3,6.36a.51.51,0,0,0-.26.44V19a.51.51,0,0,0,.26.44L13.16,25a.5.5,0,0,0,.24.06.53.53,0,0,0,.24-.06l10.3-5.6A.51.51,0,0,0,24.2,19V6.8A.51.51,0,0,0,23.94,6.36ZM13.4,24,6.46,20.22l4.91-1.75A.5.5,0,0,0,11.7,18V15.9a.51.51,0,0,0-.26-.44L8,13.6V9.44l10.8.92V13.6l-3.44,1.86a.51.51,0,0,0-.26.44V18a.51.51,0,0,0,.34.48l5.1,1.67ZM18.8,9.36,9.25,8.55,13.4,6.36,18.8,9.2Zm4.4,9.34-1.45.79L16.1,17.64V16.2l3.44-1.86a.51.51,0,0,0,.26-.44v-5a.48.48,0,0,0-.27-.44l-5.9-3.1a.47.47,0,0,0-.46,0l-5.9,3.1,0,0a.41.41,0,0,0-.14.12l0,0A.52.52,0,0,0,7,8.86s0,0,0,0v5a.51.51,0,0,0,.26.44L10.7,16.2v1.45L5.29,19.57,3.7,18.7V7.1l9.7-5.33L23.2,7.1Z"
        transform="translate(-2.7 -0.7)" />
      <path class="fill cls-1" d="M16.05,12.69a.5.5,0,0,0-.66-.25l-2,.91-2-.91a.5.5,0,0,0-.66.25.49.49,0,0,0,.24.66l2.2,1a.57.57,0,0,0,.42,0l2.2-1A.49.49,0,0,0,16.05,12.69Z"
        transform="translate(-2.7 -0.7)" />
    </svg>

    <!-- no-transactions -->
    <svg class="icon no-transactions" v-if="name === 'no-transactions'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M13.09,9.31l-2.5,1.5a.49.49,0,0,0-.24.42v3a.5.5,0,0,0,.24.43l2.5,1.5a.52.52,0,0,0,.52,0l2.5-1.5a.5.5,0,0,0,.24-.43v-3a.49.49,0,0,0-.24-.42l-2.5-1.5A.49.49,0,0,0,13.09,9.31ZM15.35,14l-2,1.2-2-1.2V11.52l2-1.2,2,1.2Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M23.85,17.23a.5.5,0,0,0-.5.5v.7l-.62.33a.5.5,0,0,0,.23.94.53.53,0,0,0,.24-.06l.88-.46a.5.5,0,0,0,.27-.45v-1A.5.5,0,0,0,23.85,17.23Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M17.5,21.5l-1.75.91a.51.51,0,0,0-.21.68.5.5,0,0,0,.44.27.46.46,0,0,0,.23-.06L18,22.38a.5.5,0,0,0,.21-.67A.49.49,0,0,0,17.5,21.5Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M21,19.67l-1.75.91a.51.51,0,0,0-.21.68.5.5,0,0,0,.44.27.46.46,0,0,0,.23-.06l1.75-.91a.5.5,0,0,0-.46-.89Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M14,23.33l-.65.34-.65-.34a.5.5,0,1,0-.47.88l.89.47a.55.55,0,0,0,.46,0l.89-.47a.5.5,0,1,0-.47-.88Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M7.46,20.58l-1.75-.91a.5.5,0,0,0-.67.21.51.51,0,0,0,.21.68L7,21.47a.46.46,0,0,0,.23.06.5.5,0,0,0,.44-.27A.51.51,0,0,0,7.46,20.58Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M11,22.41,9.2,21.5a.5.5,0,0,0-.46.88l1.75.92a.46.46,0,0,0,.23.06.5.5,0,0,0,.44-.27A.51.51,0,0,0,11,22.41Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M4,18.76l-.62-.33v-.7a.5.5,0,0,0-1,0v1a.5.5,0,0,0,.27.45l.88.46a.53.53,0,0,0,.24.06A.5.5,0,0,0,4,18.76Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M2.85,12.23a.5.5,0,0,0,.5-.5v-2a.5.5,0,0,0-1,0v2A.5.5,0,0,0,2.85,12.23Z" transform="translate(-2.35 -0.73)"
      />
      <path class="fill cls-1" d="M2.85,16.23a.5.5,0,0,0,.5-.5v-2a.5.5,0,0,0-1,0v2A.5.5,0,0,0,2.85,16.23Z" transform="translate(-2.35 -0.73)"
      />
      <path class="fill cls-1" d="M2.85,8.23a.5.5,0,0,0,.5-.5V7L4,6.71a.5.5,0,1,0-.47-.88l-.88.46a.5.5,0,0,0-.27.44v1A.5.5,0,0,0,2.85,8.23Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M7,4l-1.75.91A.51.51,0,0,0,5,5.59a.5.5,0,0,0,.44.27.46.46,0,0,0,.23-.06l1.75-.92A.5.5,0,1,0,7,4Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M9,4A.46.46,0,0,0,9.2,4L11,3.06a.51.51,0,0,0,.21-.68.5.5,0,0,0-.67-.21l-1.75.91a.51.51,0,0,0-.21.68A.5.5,0,0,0,9,4Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M12.46,2.2a.53.53,0,0,0,.24-.06l.65-.34.65.34a.5.5,0,1,0,.47-.88L13.58.79a.53.53,0,0,0-.46,0l-.89.47a.5.5,0,0,0,.23.94Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M18,3.08l-1.75-.91a.5.5,0,0,0-.67.21.51.51,0,0,0,.21.68L17.5,4a.46.46,0,0,0,.23.06.5.5,0,0,0,.44-.27A.51.51,0,0,0,18,3.08Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M21.45,4.91,19.7,4a.49.49,0,0,0-.67.21.5.5,0,0,0,.21.67L21,5.8a.46.46,0,0,0,.23.06.5.5,0,0,0,.23-1Z"
        transform="translate(-2.35 -0.73)" />
      <path class="fill cls-1" d="M24.08,6.29l-.88-.46a.5.5,0,1,0-.47.88l.62.33v.69a.5.5,0,0,0,1,0v-1A.5.5,0,0,0,24.08,6.29Z" transform="translate(-2.35 -0.73)"
      />
      <path class="fill cls-1" d="M23.85,13.23a.5.5,0,0,0-.5.5v2a.5.5,0,0,0,1,0v-2A.5.5,0,0,0,23.85,13.23Z" transform="translate(-2.35 -0.73)"
      />
      <path class="fill cls-1" d="M23.85,9.23a.5.5,0,0,0-.5.5v2a.5.5,0,0,0,1,0v-2A.5.5,0,0,0,23.85,9.23Z" transform="translate(-2.35 -0.73)"
      />
    </svg>

    <!-- logout -->
    <svg class="icon logout" v-if="name === 'logout'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17.36 17">
      <path class="fill cls-1" d="M11.75,20.5a.5.5,0,0,1-.24-.06l-8.18-4.5a.51.51,0,0,1-.26-.44v-9a.5.5,0,0,1,.5-.5.5.5,0,0,1,.5.5v8.7l7.68,4.23,7.68-4.23V6.5a.5.5,0,0,1,.5-.5.5.5,0,0,1,.5.5v9a.51.51,0,0,1-.26.44L12,20.44A.5.5,0,0,1,11.75,20.5Z"
        transform="translate(-3.07 -3.5)" />
      <path class="fill cls-1" d="M16.1,9.65a.48.48,0,0,0-.7,0l-3.15,3.14V4a.5.5,0,0,0-1,0v8.79L8.1,9.65a.49.49,0,0,0-.7.7l4,4a.36.36,0,0,0,.16.11.47.47,0,0,0,.38,0,.36.36,0,0,0,.16-.11l4-4A.48.48,0,0,0,16.1,9.65Z"
        transform="translate(-3.07 -3.5)" />
    </svg>

    <!-- no-contacts -->
    <svg class="icon no-contacts" v-if="name === 'no-contacts'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M6.57,14a.5.5,0,0,0,.24.43l3.94,2.33a.5.5,0,0,0,.5,0l3.94-2.33a.5.5,0,0,0,.24-.43v-3.8a.49.49,0,0,0-.24-.42l-4-2.44a.53.53,0,0,0-.52,0L6.78,9.63h0a.44.44,0,0,0-.13.14h0a.63.63,0,0,0-.08.23ZM11,15.7l-3.43-2v-3l6.86.68v2.33Zm0-7.4,3.31,2L8.57,9.7Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M18.88,18.85a.46.46,0,0,0-.23.06l-1.76.91a.5.5,0,0,0-.22.67.51.51,0,0,0,.68.22l1.75-.89h0a.49.49,0,0,0,.17-.67A.5.5,0,0,0,18.88,18.85Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M15.42,20.73a.45.45,0,0,0-.23,0l-1.75.92a.49.49,0,0,0-.18.59h0a.49.49,0,0,0,.64.28l1.75-.91a.49.49,0,0,0-.23-.88Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M5.17,19.82l-1.76-.91a.46.46,0,0,0-.23-.06.51.51,0,0,0-.51.51.5.5,0,0,0,.24.43h0l1.75.89a.51.51,0,0,0,.68-.22A.49.49,0,0,0,5.17,19.82Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M8.62,21.65l-1.75-.92a.45.45,0,0,0-.23,0,.49.49,0,0,0-.41.56h0a.49.49,0,0,0,.18.32l1.75.91a.49.49,0,0,0,.64-.27h0A.48.48,0,0,0,8.62,21.65Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M21.55,16.5H21.5a.5.5,0,0,0-.5.5v.7l-.62.32a.51.51,0,0,0-.2.68h0a.48.48,0,0,0,.43.26.49.49,0,0,0,.24,0l.88-.47a.48.48,0,0,0,.27-.44v-1a.5.5,0,0,0-.45-.55Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M11.65,22.59l-.65.35-.65-.35a.52.52,0,0,0-.68.21.51.51,0,0,0,.21.68l.89.46a.47.47,0,0,0,.46,0l.89-.46a.51.51,0,0,0,.21-.68.5.5,0,0,0-.68-.21Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M1.65,18,1,17.7V17a.5.5,0,0,0-.5-.5A.5.5,0,0,0,0,17v1a.48.48,0,0,0,.27.44l.88.47a.49.49,0,0,0,.24,0,.51.51,0,0,0,.44-.26A.5.5,0,0,0,1.65,18Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M.5,15.5A.5.5,0,0,0,1,15V13a.5.5,0,0,0-.5-.5A.5.5,0,0,0,0,13v2A.5.5,0,0,0,.5,15.5Z" transform="translate(0 0)"
      />
      <path class="fill cls-1" d="M.5,11.5A.5.5,0,0,0,1,11V9a.5.5,0,0,0-.5-.5A.5.5,0,0,0,0,9v2A.5.5,0,0,0,.5,11.5Z" transform="translate(0 0)"
      />
      <path class="fill cls-1" d="M1.18,5.11.3,5.56A.48.48,0,0,0,0,6V7a.51.51,0,0,0,.5.5A.5.5,0,0,0,1,7V6.3L1.65,6a.52.52,0,0,0,.21-.68.5.5,0,0,0-.68-.21Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M10.11,1.46a.52.52,0,0,0,.24-.05L11,1.06l.65.35a.52.52,0,0,0,.68-.21.52.52,0,0,0-.21-.68L11.23.06a.47.47,0,0,0-.46,0L9.88.52a.51.51,0,0,0-.21.68A.51.51,0,0,0,10.11,1.46Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M4.71,3.26,3,4.15H3a.5.5,0,0,0-.17.67.53.53,0,0,0,.44.27A.46.46,0,0,0,3.5,5l1.76-.91a.5.5,0,0,0,.21-.67.49.49,0,0,0-.67-.22Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M6.64,3.24a.45.45,0,0,0,.23,0l1.75-.92a.49.49,0,0,0,.18-.59h0a.49.49,0,0,0-.63-.28h0l-1.75.91a.51.51,0,0,0-.06.72.48.48,0,0,0,.29.17Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M19.15,4.18h0L17.4,3.29a.5.5,0,1,0-.46.89h0l1.76.91a.46.46,0,0,0,.23.06.54.54,0,0,0,.44-.27.49.49,0,0,0-.17-.67Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M13.9,1.44a.49.49,0,0,0-.64.28h0a.49.49,0,0,0,.18.59l1.75.92a.45.45,0,0,0,.23,0,.5.5,0,0,0,.23-.89Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M21.73,5.56l-.88-.47a.52.52,0,0,0-.68.21.51.51,0,0,0,.21.68L21,6.3V7a.5.5,0,0,0,.5.5A.5.5,0,0,0,22,7V6A.48.48,0,0,0,21.73,5.56Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M21.5,12.5a.5.5,0,0,0-.5.5v2a.5.5,0,0,0,.5.5A.5.5,0,0,0,22,15V13A.5.5,0,0,0,21.5,12.5Z" transform="translate(0 0)"
      />
      <path class="fill cls-1" d="M21.5,8.5A.5.5,0,0,0,21,9v2a.5.5,0,0,0,.5.5A.5.5,0,0,0,22,11V9A.5.5,0,0,0,21.5,8.5Z" transform="translate(0 0)"
      />
    </svg>

    <!-- unconfirmed -->
    <svg class="icon unconfirmed" v-if="name === 'unconfirmed'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.51 24.49">
      <path class="fill cls-1" d="M22,5.47,11.47,0a.71.71,0,0,0-.69,0L.28,5.47a.73.73,0,0,0-.41.66v12a.73.73,0,0,0,.41.66l10.49,5.5a.69.69,0,0,0,.34.09l.15,0a.76.76,0,0,0,.21,0L22,18.79a.73.73,0,0,0,.41-.66v-12A.73.73,0,0,0,22,5.47ZM20.88,6.59V17.67l-9.76,5.12L1.37,17.67V6.59l9.75-5.11Z"
        transform="translate(0.13 0.12)" />
      <path class="fill cls-1" d="M11.11,11,8.28,8.2a.77.77,0,0,0-1.08,0,.78.78,0,0,0,0,1.08L10,12.11,7.2,14.94A.79.79,0,0,0,7.2,16a.77.77,0,0,0,1.08,0l2.83-2.83L14,16.05a.78.78,0,0,0,1.08,0,.75.75,0,0,0,.22-.54.76.76,0,0,0-.22-.54l-2.87-2.86L15,9.28A.76.76,0,1,0,13.94,8.2Z"
        transform="translate(0.13 0.12)" />
    </svg>

    <!-- unconfirmed-big -->
    <svg class="icon unconfirmed-big" v-if="name === 'unconfirmed-big'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.26 24.24">
      <path class="fill cls-1" d="M12.12,24.22a.6.6,0,0,1-.29-.08l-10.5-5.5A.61.61,0,0,1,1,18.09v-12a.61.61,0,0,1,.34-.55L11.83.07a.6.6,0,0,1,.58,0l10.5,5.5a.61.61,0,0,1,.34.55v12a.61.61,0,0,1-.34.55l-10.5,5.55A.71.71,0,0,1,12.12,24.22ZM2.24,17.74l9.88,5.18L22,17.74V6.5L12.12,1.33,2.24,6.5Z"
        transform="translate(-0.99 0)" />
      <path class="fill cls-1" d="M15.06,8.79l-2.92,2.92L9.22,8.79a.64.64,0,1,0-.9.9l2.92,2.92-3,3a.65.65,0,0,0,0,.9.63.63,0,0,0,.9,0l3-3,2.92,2.92a.63.63,0,0,0,.89-.9L13,12.61,16,9.69a.63.63,0,0,0,0-.9A.62.62,0,0,0,15.06,8.79Z"
        transform="translate(-0.99 0)" />
    </svg>

    <!-- ico -->
    <svg class="icon ico" v-if="name === 'ico'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 22.75">
      <path class="fill cls-1" d="M17.71,6.25,21.35,2.6a.47.47,0,0,0,.11-.54A.5.5,0,0,0,21,1.75H9.5V.5a.5.5,0,0,0-1,0v13L.77,17.56A.49.49,0,0,0,.5,18a.5.5,0,0,0,.27.44l8,4.25a.47.47,0,0,0,.46,0l8-4.25A.5.5,0,0,0,17.5,18a.49.49,0,0,0-.27-.44L9.5,13.45v-2.7H21a.5.5,0,0,0,.46-.31.47.47,0,0,0-.11-.54ZM15.93,18,9,21.68,2.07,18,8.5,14.58v3.67a.5.5,0,0,0,1,0V14.58ZM9.5,9.75v-7H19.79L16.65,5.9a.48.48,0,0,0,0,.7l3.14,3.15Z"
        transform="translate(-0.5)" />
    </svg>

    <!-- dex -->
    <svg class="icon dex" v-if="name === 'dex'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M22.29,5.66,11.79.16a.53.53,0,0,0-.46,0L.83,5.66a.52.52,0,0,0-.27.45v12a.52.52,0,0,0,.27.44l10.5,5.5a.47.47,0,0,0,.46,0l10.5-5.5a.5.5,0,0,0,.27-.44v-12A.5.5,0,0,0,22.29,5.66ZM21.56,17.8,11.56,23l-10-5.24V6.41l10-5.24,10,5.24Z"
        transform="translate(-0.56 -0.11)" />
      <path class="fill cls-1" d="M13.69,12.61h3L14.38,15a.5.5,0,0,0,0,.7.5.5,0,0,0,.71,0l3.2-3.2a.42.42,0,0,0,.11-.16.5.5,0,0,0,0-.38.39.39,0,0,0-.11-.17l-3.2-3.2a.5.5,0,0,0-.35-.15.51.51,0,0,0-.36.15.51.51,0,0,0,0,.71l2.35,2.35H6.39L8.74,9.26a.5.5,0,0,0,0-.71.48.48,0,0,0-.7,0l-3.21,3.2a.71.71,0,0,0-.11.17.5.5,0,0,0,0,.38.91.91,0,0,0,.11.16L8,15.66a.49.49,0,1,0,.7-.7L6.39,12.61h7.3Z"
        transform="translate(-0.56 -0.11)" />
    </svg>

    <!-- transactions -->
    <svg class="icon transactions" v-if="name === 'transactions'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.25 13.25">
      <path class="fill" d="M1,5.25a.5.5,0,0,1,.15-.35l4-4a.5.5,0,1,1,.71.71L2.21,5.25,5.85,8.9a.5.5,0,0,1-.71.71l-4-4A.5.5,0,0,1,1,5.25Z"
        transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M5.5,9.88a.63.63,0,0,1-.44-.18l-4-4a.62.62,0,0,1,0-.88l4-4a.62.62,0,0,1,.88.88L2.38,5.25,5.94,8.81A.63.63,0,0,1,5.5,9.88Zm0-9A.37.37,0,0,0,5.23,1l-4,4a.37.37,0,0,0,0,.53l4,4A.37.37,0,0,0,5.77,9L2,5.25,5.77,1.52A.38.38,0,0,0,5.5.88Z"
        transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M14.5,4.75H1.5a.5.5,0,0,0,0,1h13a.5.5,0,0,0,0-1Z" transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M14.5,5.88H1.5a.63.63,0,0,1,0-1.25h13a.63.63,0,0,1,0,1.25Zm-13-1a.38.38,0,0,0,0,.75h13a.38.38,0,0,0,0-.75Z" transform="translate(-0.88 -0.63)"
      />
      <path class="fill" d="M20,13.25a.5.5,0,0,1,.15-.35l3.65-3.65L20.15,5.6a.5.5,0,0,1,.71-.71l4,4a.5.5,0,0,1,0,.71l-4,4a.5.5,0,0,1-.85-.35Z"
        transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M20.5,13.88a.63.63,0,0,1-.44-1.07l3.56-3.56L20.06,5.69a.63.63,0,1,1,.88-.88l4,4a.63.63,0,0,1,0,.88l-4,4A.63.63,0,0,1,20.5,13.88Zm0-9a.38.38,0,0,0-.26.64L24,9.25l-.09.09L20.24,13a.37.37,0,0,0,0,.53.37.37,0,0,0,.53,0l4-4a.37.37,0,0,0,0-.53l-4-4A.37.37,0,0,0,20.5,4.88Z"
        transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M24.5,8.75h-13a.5.5,0,0,0,0,1h13a.5.5,0,0,0,0-1Z" transform="translate(-0.88 -0.63)" />
      <path class="fill" d="M24.5,9.88h-13a.63.63,0,0,1,0-1.25h13a.63.63,0,0,1,0,1.25Zm-13-1a.38.38,0,0,0,0,.75h13a.38.38,0,0,0,0-.75Z" transform="translate(-0.88 -0.63)"
      />
    </svg>

    <!-- holdings -->
    <svg class="icon holdings" v-if="name === 'holdings'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.25 24.25">
      <path class="fill" d="M12.88,19.25a.5.5,0,0,1-.35-.15l-6-6a.5.5,0,0,1,0-.71l6-6a.5.5,0,0,1,.71,0l6,6a.5.5,0,0,1,0,.71l-6,6A.5.5,0,0,1,12.88,19.25Zm-5.29-6.5L12.88,18l5.29-5.29L12.88,7.46Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M12.88,19.38a.62.62,0,0,1-.44-.18l-6-6a.63.63,0,0,1,0-.88l6-6a.63.63,0,0,1,.88,0l6,6a.63.63,0,0,1,0,.88l-6,6A.63.63,0,0,1,12.88,19.38Zm0-13a.37.37,0,0,0-.27.11l-6,6a.38.38,0,0,0,0,.53l6,6a.38.38,0,0,0,.53,0l6-6a.37.37,0,0,0,0-.53l-6-6A.37.37,0,0,0,12.88,6.38Zm0,11.84-.09-.09L7.41,12.75l5.47-5.47.09.09,5.38,5.38-.09.09ZM7.76,12.75l5.12,5.12L18,12.75,12.88,7.63Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M6.88,7.25a.5.5,0,0,1-.35-.15l-4-4a.5.5,0,0,1,.71-.71l4,4a.5.5,0,0,1-.35.85Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M6.88,7.38a.62.62,0,0,1-.44-.18l-4-4a.62.62,0,0,1,.88-.88l4,4a.63.63,0,0,1-.44,1.07Zm-4-5A.38.38,0,0,0,2.61,3l4,4a.38.38,0,0,0,.53,0,.38.38,0,0,0,0-.53l-4-4A.37.37,0,0,0,2.88,2.38Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M12.88,4.25a.5.5,0,0,1-.5-.5V1.25a.5.5,0,0,1,1,0v2.5A.5.5,0,0,1,12.88,4.25Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M12.88,4.38a.63.63,0,0,1-.62-.63V1.25a.63.63,0,0,1,1.25,0v2.5A.63.63,0,0,1,12.88,4.38Zm0-3.5a.38.38,0,0,0-.37.38v2.5a.38.38,0,0,0,.75,0V1.25A.38.38,0,0,0,12.88.88Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M2.88,23.25a.5.5,0,0,1-.35-.85l4-4a.5.5,0,1,1,.71.71l-4,4A.5.5,0,0,1,2.88,23.25Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M2.88,23.38a.63.63,0,0,1-.44-1.07l4-4a.63.63,0,1,1,.88.88l-4,4A.62.62,0,0,1,2.88,23.38Zm4-5a.37.37,0,0,0-.27.11l-4,4a.37.37,0,0,0,0,.53.38.38,0,0,0,.53,0l4-4a.38.38,0,0,0-.27-.64Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M12.88,24.75a.5.5,0,0,1-.5-.5v-2.5a.5.5,0,1,1,1,0v2.5A.5.5,0,0,1,12.88,24.75Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M12.88,24.88a.63.63,0,0,1-.62-.62v-2.5a.62.62,0,1,1,1.25,0v2.5A.63.63,0,0,1,12.88,24.88Zm0-3.5a.38.38,0,0,0-.37.38v2.5a.37.37,0,0,0,.75,0v-2.5A.38.38,0,0,0,12.88,21.38Z"
        transform="translate(-0.75 -0.63)" />
      <v d="M3.88,13.25H1.38a.5.5,0,0,1,0-1h2.5a.5.5,0,0,1,0,1Z" transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M3.88,13.38H1.38a.63.63,0,0,1,0-1.25h2.5a.63.63,0,0,1,0,1.25Zm-2.5-1a.38.38,0,0,0,0,.75h2.5a.38.38,0,0,0,0-.75Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M18.88,7.25a.5.5,0,0,1-.35-.85l4-4a.5.5,0,0,1,.71.71l-4,4A.5.5,0,0,1,18.88,7.25Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M18.88,7.38a.63.63,0,0,1-.44-1.07l4-4a.63.63,0,0,1,.88.88l-4,4A.62.62,0,0,1,18.88,7.38Zm4-5a.37.37,0,0,0-.26.11l-4,4a.37.37,0,0,0,0,.53.38.38,0,0,0,.53,0l4-4a.38.38,0,0,0-.26-.64Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M22.88,23.25a.5.5,0,0,1-.35-.15l-4-4a.5.5,0,0,1,.71-.71l4,4a.5.5,0,0,1-.35.85Z" transform="translate(-0.75 -0.63)"
      />
      <path class="fill" d="M22.88,23.38a.63.63,0,0,1-.44-.18l-4-4a.63.63,0,1,1,.88-.88l4,4a.63.63,0,0,1-.44,1.07Zm-4-5a.38.38,0,0,0-.26.64l4,4a.38.38,0,0,0,.53,0,.37.37,0,0,0,0-.53l-4-4A.37.37,0,0,0,18.88,18.38Z"
        transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M24.38,13.25h-2.5a.5.5,0,0,1,0-1h2.5a.5.5,0,0,1,0,1Z" transform="translate(-0.75 -0.63)" />
      <path class="fill" d="M24.38,13.38h-2.5a.63.63,0,0,1,0-1.25h2.5a.62.62,0,0,1,0,1.25Zm-2.5-1a.37.37,0,0,0,0,.75h2.5a.38.38,0,0,0,0-.75Z"
        transform="translate(-0.75 -0.63)" />
    </svg>

    <!-- contacts -->
    <svg class="icon contacts" v-if="name === 'contacts'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 11.29 12.92">
      <path class="fill cls-1" d="M5.64,6.83a.37.37,0,0,1-.19-.05L4.18,6A.37.37,0,0,1,4,5.72V4.24a.37.37,0,0,1,.19-.32l1.28-.74a.37.37,0,0,1,.38,0l1.28.74a.37.37,0,0,1,.19.32V5.72A.37.37,0,0,1,7.11,6l-1.28.74A.37.37,0,0,1,5.64,6.83ZM4.74,5.5l.9.52.9-.52v-1l-.9-.52-.9.52Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M3,8.75A.38.38,0,0,1,2.82,8L5.46,6.53a.37.37,0,0,1,.38,0L8.47,8a.38.38,0,0,1,.14.51.38.38,0,0,1-.51.14L5.64,7.28,3.19,8.7A.38.38,0,0,1,3,8.75Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M5.64,12.92a.38.38,0,0,1-.19,0l-5.27-3A.38.38,0,0,1,0,9.5V3.42a.37.37,0,0,1,.19-.32L5.46,0a.37.37,0,0,1,.38,0l5.27,3a.37.37,0,0,1,.19.32V9.5a.38.38,0,0,1-.19.33l-5.27,3A.38.38,0,0,1,5.64,12.92ZM.75,9.28l4.89,2.83,4.89-2.83V3.63L5.64.81.75,3.63Z"
        transform="translate(0 0)" />
    </svg>

    <!-- feedback -->
    <svg class="icon feedback" v-if="name === 'feedback'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.5 17.5">
      <path class="fill cls-1" d="M23.75,17.5H.75A.75.75,0,0,1,0,16.75V.75A.75.75,0,0,1,.75,0h23a.75.75,0,0,1,.75.75v16A.75.75,0,0,1,23.75,17.5ZM1.5,16H23V1.5H1.5Z"
        transform="translate(0)" />
      <path class="fill cls-1" d="M12.25,11a.75.75,0,0,1-.41-.12L.34,3.38a.75.75,0,0,1,.82-1.26L12.25,9.35,23.34,2.12a.75.75,0,0,1,.82,1.26l-11.5,7.5A.75.75,0,0,1,12.25,11Z"
        transform="translate(0)" />
      <path class="fill cls-1" d="M4.75,12.5a.75.75,0,0,1-.47-1.34l4.4-3.52a.75.75,0,0,1,.94,1.17l-4.4,3.52A.75.75,0,0,1,4.75,12.5Z"
        transform="translate(0)" />
      <path class="fill cls-1" d="M19.75,12.5a.75.75,0,0,1-.47-.16l-4.4-3.52a.75.75,0,1,1,.94-1.17l4.4,3.52a.75.75,0,0,1-.47,1.34Z"
        transform="translate(0)" />
    </svg>

    <!-- lock -->
    <svg class="icon lock" v-if="name === 'lock'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16.22 24.83">
      <path class="fill cls-1" d="M15.95,11.21l-1.67-.88V4.76a.5.5,0,0,0-.2-.4L8.46.1a.5.5,0,0,0-.6,0L2.64,4.05a.5.5,0,0,0-.2.4v5.62L.27,11.21a.5.5,0,0,0-.27.44v8.7a.5.5,0,0,0,.27.44l7.61,4a.5.5,0,0,0,.46,0l7.61-4a.5.5,0,0,0,.27-.44v-8.7A.5.5,0,0,0,15.95,11.21ZM3.44,4.7,8.15,1.13,13.28,5v4.8L8.34,7.22a.5.5,0,0,0-.46,0L3.44,9.55ZM15.22,20,8.11,23.77,1,20V12L8.11,8.23,15.22,12Z"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M7.75,12.4l-1.5,1.5a.5.5,0,0,0,0,.71L7.61,16v3.63a.5.5,0,0,0,1,0V16L10,14.6a.5.5,0,0,0,0-.71l-1.5-1.5A.51.51,0,0,0,7.75,12.4ZM8.9,14.25,8.11,15l-.79-.79.79-.79Z"
        transform="translate(0 0)" />
    </svg>

    <!-- expand -->
    <svg class="icon expand" v-if="name === 'expand'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12.19 12.23">
      <path class="fill cls-1" d="M11.26,6.58a.5.5,0,0,1-.14-.35l.07-5.16L6,1A.5.5,0,1,1,6,0L11.7.08a.5.5,0,0,1,.49.51l-.08,5.66a.5.5,0,0,1-.86.34"
        transform="translate(0 0)" />
      <path class="fill cls-1" d="M3.45,8.6a.5.5,0,0,1,0-.71L11.34.22a.5.5,0,1,1,.7.72L4.16,8.61a.5.5,0,0,1-.71,0" transform="translate(0 0)"
      />
      <path class="fill cls-1" d="M8.87,12.23H.5a.5.5,0,0,1-.5-.5V3.37a.5.5,0,0,1,.5-.5h5a.5.5,0,0,1,0,1H1v7.36H8.37V6.73a.5.5,0,0,1,1,0v5A.5.5,0,0,1,8.87,12.23Z"
        transform="translate(0 0)" />
    </svg>

    <!-- about -->
    <svg class="icon about" v-if="name === 'about'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 11.29 12.92">
      <path class="fill cls-1" d="M-34.23,8a.41.41,0,0,1-.19,0l-1.28-.74a.37.37,0,0,1-.19-.33V5.38a.37.37,0,0,1,.19-.33l1.28-.74a.4.4,0,0,1,.37,0l1.28.74a.37.37,0,0,1,.19.33V6.85a.37.37,0,0,1-.19.33L-34,7.92A.41.41,0,0,1-34.23,8Zm-.91-1.33.91.52.9-.52v-1l-.9-.52-.91.52Z"
        transform="translate(39.88 0.34)" />
      <path class="fill cls-1" d="M-34.23,12.57a.32.32,0,0,1-.19-.05l-5.27-3a.37.37,0,0,1-.19-.32V3.07a.37.37,0,0,1,.19-.32l5.27-3a.35.35,0,0,1,.37,0l5.27,3a.37.37,0,0,1,.19.32V9.16a.37.37,0,0,1-.19.32l-5.27,3A.31.31,0,0,1-34.23,12.57Zm-4.9-3.63,4.9,2.83,4.89-2.83V3.29L-34.23.47l-4.9,2.82Z"
        transform="translate(39.88 0.34)" />
    </svg>

    <!-- claim-gas -->
    <svg class="icon claim-gas" v-if="name === 'claim-gas'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
      <path class="fill cls-1" d="M89.65,328.85a.48.48,0,0,0,.7,0l4-4a.48.48,0,0,0,0-.7l-7-7a.48.48,0,0,0-.7,0L83,320.79l-1.65-1.64a.48.48,0,0,0-.7,0L75,324.79l-1.65-1.64a.49.49,0,0,0-.7.7l2,2,7,7,8,8a.48.48,0,0,0,.7,0,.48.48,0,0,0,0-.7l-7.64-7.65L88,327.21ZM87,318.21l6.29,6.29L90,327.79l-1.65-1.64-4.64-4.65Zm-5,13.58-6.29-6.29L81,320.21l1.65,1.64,4.64,4.65Z"
        transform="translate(-72.5 -317)" />
    </svg>

    <!-- commit -->
    <svg class="icon commit" v-if="name === 'commit'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 22">
      <path class="fill" d="M419.35,421.15a.36.36,0,0,0-.16-.11.5.5,0,0,0-.38,0,.36.36,0,0,0-.16.11l-4,4a.49.49,0,0,0,.7.7l3.15-3.14V434.5a.5.5,0,0,0,1,0V422.71l3.15,3.14a.48.48,0,0,0,.7,0,.48.48,0,0,0,0-.7Z" transform="translate(-407.5 -413)"/>
      <path class="fill" d="M431.35,420.65l-2.5-2.5a.47.47,0,0,0-.35-.15h-3v-1.5a.47.47,0,0,0-.15-.35l-3-3A.47.47,0,0,0,422,413h-5a.47.47,0,0,0-.35.15l-3,3a.47.47,0,0,0-.15.35V418H411a.47.47,0,0,0-.35.15l-3,3a.47.47,0,0,0-.15.35v4a.47.47,0,0,0,.15.35l3,3a.47.47,0,0,0,.35.15h2a.5.5,0,0,0,0-1h-1.79l-2.71-2.71v-3.58l2.71-2.71H414a.5.5,0,0,0,.5-.5v-1.79l2.71-2.71h4.58l2.71,2.71v1.79a.5.5,0,0,0,.5.5h3.29l2.21,2.21v4.08L427.79,428H426a.5.5,0,0,0,0,1h2a.47.47,0,0,0,.35-.15l3-3a.47.47,0,0,0,.15-.35V421A.47.47,0,0,0,431.35,420.65Z" transform="translate(-407.5 -413)"/>
    </svg>

    <!-- compound -->
    <svg class="icon compound" v-if="name === 'compound'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      <path class="fill" d="M379.44,429h-2a.5.5,0,0,1,0-1h1.79l2.71-2.71v-4.08L379.73,419h-3.29a.5.5,0,0,1-.5-.5v-1.79L373.23,414h-4.59l-2.7,2.71v1.79a.5.5,0,0,1-.5.5h-2.8l-2.7,2.71v3.58l2.7,2.71h1.8a.5.5,0,1,1,0,1h-2a.49.49,0,0,1-.36-.15l-3-3a.5.5,0,0,1-.14-.35v-4a.5.5,0,0,1,.14-.35l3-3a.49.49,0,0,1,.36-.15h2.5v-1.5a.5.5,0,0,1,.14-.35l3-3a.49.49,0,0,1,.36-.15h5a.47.47,0,0,1,.35.15l3,3a.51.51,0,0,1,.15.35V418h3a.47.47,0,0,1,.35.15l2.5,2.5a.51.51,0,0,1,.15.35v4.5a.51.51,0,0,1-.15.35l-3,3A.47.47,0,0,1,379.44,429Z" transform="translate(-358.94 -413)"/>
      <path class="fill" d="M370.94,432a.49.49,0,0,1,.35.15l4,4a.48.48,0,0,1,0,.7.5.5,0,0,1-.71,0l-3.64-3.64-3.65,3.64a.5.5,0,0,1-.71,0,.5.5,0,0,1,0-.7l4-4A.51.51,0,0,1,370.94,432Z" transform="translate(-358.94 -413)"/>
      <path class="fill" d="M370.94,426.5a.49.49,0,0,1,.35.15l4,4a.48.48,0,0,1,0,.7.5.5,0,0,1-.71,0l-3.64-3.64-3.65,3.64a.5.5,0,0,1-.71,0,.5.5,0,0,1,0-.7l4-4A.51.51,0,0,1,370.94,426.5Z" transform="translate(-358.94 -413)"/>
      <path class="fill" d="M370.94,421a.49.49,0,0,1,.35.15l4,4a.48.48,0,0,1,0,.7.5.5,0,0,1-.71,0l-3.64-3.64-3.65,3.64a.5.5,0,0,1-.71,0,.5.5,0,0,1,0-.7l4-4A.51.51,0,0,1,370.94,421Z" transform="translate(-358.94 -413)"/>
    </svg>

    <!-- claim -->
    <svg class="icon claim" v-if="name === 'claim'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      <path class="fill" d="M383.35,420.65l-2.5-2.5a.47.47,0,0,0-.35-.15h-3v-1.5a.47.47,0,0,0-.15-.35l-3-3A.47.47,0,0,0,374,413h-5a.47.47,0,0,0-.35.15l-3,3a.47.47,0,0,0-.15.35V418H363a.47.47,0,0,0-.35.15l-3,3a.47.47,0,0,0-.15.35v4a.47.47,0,0,0,.15.35l3,3a.47.47,0,0,0,.35.15h2a.5.5,0,0,0,0-1h-1.79l-2.71-2.71v-3.58l2.71-2.71H366a.5.5,0,0,0,.5-.5v-1.79l2.71-2.71h4.58l2.71,2.71v1.79a.5.5,0,0,0,.5.5h3.29l2.21,2.21v4.08L379.79,428H378a.5.5,0,0,0,0,1h2a.47.47,0,0,0,.35-.15l3-3a.47.47,0,0,0,.15-.35V421A.47.47,0,0,0,383.35,420.65Z" transform="translate(-359.5 -413)"/>
      <path class="fill" d="M374.65,432.15l-3.15,3.14V423.5a.5.5,0,0,0-1,0v11.79l-3.15-3.14a.49.49,0,0,0-.7.7l4,4a.36.36,0,0,0,.16.11.47.47,0,0,0,.38,0,.36.36,0,0,0,.16-.11l4-4a.49.49,0,0,0-.7-.7Z" transform="translate(-359.5 -413)"/>
    </svg>

    <!-- hex -->
    <svg class="icon hex" v-if="name === 'hex'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.75 24.75">
      <path class="fill" d="M157.35,401.1l-10.51-5.5a.89.89,0,0,0-.81,0l-10.5,5.5a.87.87,0,0,0-.47.78v12a.87.87,0,0,0,.47.77l10.5,5.5a.83.83,0,0,0,.41.1.79.79,0,0,0,.4-.1l10.51-5.5a.88.88,0,0,0,.46-.77v-12A.88.88,0,0,0,157.35,401.1Z" transform="translate(-135.06 -395.5)"/>
    </svg>

    <!-- star -->
    <svg class="icon star" v-if="name === 'star'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      <path class="fill" d="M467.5,431.5a.47.47,0,0,1-.35-.15l-6-6a.48.48,0,0,1,0-.7l6-6a.48.48,0,0,1,.7,0l6,6a.48.48,0,0,1,0,.7l-6,6A.47.47,0,0,1,467.5,431.5Zm-5.29-6.5,5.29,5.29,5.29-5.29-5.29-5.29Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M461.5,419.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7l4,4a.48.48,0,0,1,0,.7A.47.47,0,0,1,461.5,419.5Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M467.5,416.5a.5.5,0,0,1-.5-.5v-2.5a.5.5,0,0,1,1,0V416A.5.5,0,0,1,467.5,416.5Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M457.5,435.5a.47.47,0,0,1-.35-.15.48.48,0,0,1,0-.7l4-4a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,457.5,435.5Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M467.5,437a.5.5,0,0,1-.5-.5V434a.5.5,0,0,1,1,0v2.5A.5.5,0,0,1,467.5,437Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M458.5,425.5H456a.5.5,0,0,1,0-1h2.5a.5.5,0,0,1,0,1Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M473.5,419.5a.47.47,0,0,1-.35-.15.48.48,0,0,1,0-.7l4-4a.49.49,0,0,1,.7.7l-4,4A.47.47,0,0,1,473.5,419.5Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M477.5,435.5a.47.47,0,0,1-.35-.15l-4-4a.49.49,0,0,1,.7-.7l4,4a.48.48,0,0,1,0,.7A.47.47,0,0,1,477.5,435.5Z" transform="translate(-455.5 -413)"/>
      <path class="fill" d="M479,425.5h-2.5a.5.5,0,0,1,0-1H479a.5.5,0,0,1,0,1Z" transform="translate(-455.5 -413)"/>
    </svg>

    <!-- award -->
    <svg class="icon award" v-if="name === 'award'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17 24">
      <path class="fill" d="M235.85,380.15l-2.35-2.36V375.5a.5.5,0,0,0-.5-.5h-2.29l-2.34-2.34,3.83-1.7a.51.51,0,0,0,.3-.46V366H234a.5.5,0,0,0,0-1H221a.5.5,0,0,0,0,1h1.5v4.5a.51.51,0,0,0,.3.46l3.83,1.7L224.29,375H222a.5.5,0,0,0-.5.5v2.29l-2.35,2.36a.48.48,0,0,0,0,.7l2.35,2.36v2.29a.5.5,0,0,0,.5.5h2.29l2.86,2.85a.48.48,0,0,0,.7,0l2.86-2.85H233a.5.5,0,0,0,.5-.5v-2.29l2.35-2.36A.48.48,0,0,0,235.85,380.15Zm-9.35-8.64V366h2v5.51l-1,.44Zm5-1.33-2,.88V366h2Zm-8-4.18h2v5.06l-2-.88Zm9.15,16.65a.47.47,0,0,0-.15.35v2h-2a.47.47,0,0,0-.35.15l-2.65,2.64-2.65-2.64a.47.47,0,0,0-.35-.15h-2v-2a.47.47,0,0,0-.15-.35l-2.14-2.15,2.14-2.15a.47.47,0,0,0,.15-.35v-2h2a.47.47,0,0,0,.35-.15l2.65-2.64,2.65,2.64a.47.47,0,0,0,.35.15h2v2a.47.47,0,0,0,.15.35l2.14,2.15Z" transform="translate(-219 -365)"/>
      <path class="fill" d="M227.76,377.07a.53.53,0,0,0-.52,0l-2.5,1.5a.5.5,0,0,0-.24.43v3a.5.5,0,0,0,.24.43l2.5,1.5a.52.52,0,0,0,.52,0l2.5-1.5a.5.5,0,0,0,.24-.43v-3a.5.5,0,0,0-.24-.43Zm1.74,4.65-2,1.2-2-1.2v-2.44l2-1.2,2,1.2Z" transform="translate(-219 -365)"/>
    </svg>

    <!-- info -->
    <svg class="icon info" v-if="name === 'info'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.75 24.75">
      <path class="fill" d="M-41.66,367.1l-10.5-5.5a.89.89,0,0,0-.81,0l-10.5,5.5a.87.87,0,0,0-.47.78v12a.87.87,0,0,0,.47.77l10.5,5.5a.83.83,0,0,0,.41.1.79.79,0,0,0,.4-.1l10.51-5.5a.88.88,0,0,0,.46-.77v-12A.87.87,0,0,0-41.66,367.1Zm-1.28,1.3v11l-9.62,5-9.63-5v-11l9.63-5Z" transform="translate(63.94 -361.5)"/>
      <polygon class="fill" points="8.75 8.49 10.38 8.49 10.38 16.12 8.75 16.12 8.75 18.12 14 18.12 14 16.12 12.38 16.12 12.38 8.49 14 8.49 14 6.5 8.75 6.5 8.75 8.49"/>
    </svg>

    <!-- radio-off -->
    <svg class="radio-off info" v-if="name === 'radio-off'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.75 24.75">
      <path class="fill cls-1" d="M-41.66,367.1l-10.5-5.5a.89.89,0,0,0-.81,0l-10.5,5.5a.87.87,0,0,0-.47.78v12a.87.87,0,0,0,.47.77l10.5,5.5a.83.83,0,0,0,.41.1.79.79,0,0,0,.4-.1l10.51-5.5a.88.88,0,0,0,.46-.77v-12A.87.87,0,0,0-41.66,367.1Zm-1.28,1.3v11l-9.62,5-9.63-5v-11l9.63-5Z"
        transform="translate(63.94 -361.5)" />
    </svg>

    <!-- radio-on -->
    <svg class="radio-on info" v-if="name === 'radio-on'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22.75 24.75">
      <path class="fill cls-1" d="M-53,368.87l-3.88,2a.86.86,0,0,0-.46.76v4.43a.86.86,0,0,0,.46.76l3.88,2,.23.09h.17l.25,0,4-2.11a.85.85,0,0,0,.46-.76v-4.43a.85.85,0,0,0-.46-.76l-3.88-2A.88.88,0,0,0-53,368.87Z"
        transform="translate(63.94 -361.5)" />
      <path class="fill cls-1" d="M-41.66,367.1l-10.5-5.5a.89.89,0,0,0-.81,0l-10.5,5.5a.87.87,0,0,0-.47.78v12a.87.87,0,0,0,.47.77l10.5,5.5a.83.83,0,0,0,.41.1.79.79,0,0,0,.4-.1l10.51-5.5a.88.88,0,0,0,.46-.77v-12A.87.87,0,0,0-41.66,367.1Zm-1.28,1.3v11l-9.62,5-9.63-5v-11l9.63-5Z"
        transform="translate(63.94 -361.5)" />
    </svg>

  </div>
</template>

<script>
export default {
  props: {
    name: {
      required: true,
      type: String,
    },
  },
};
</script>

<style lang="scss">
.aph-icon {
  font-size: 0;

  .fill {
    @include transition(fill);

    fill: white;
  }

  .logo {
    .st1 {
      fill: $purple;
    }
  }

  .logo-mark {
    .st0 {
      fill: $purple;
    }
  }

  .arrow-down {
    path {
      transform: rotate(-90deg) translate(-21px, -3px);
    }
  }

  .arrow-right {
    path {
      transform: rotate(180deg) translate(-21px, -17px);
    }
  }

  .cancel {
    path {
      &.x {
        transform-origin: center;
        transform: rotate(-45deg);
      }
    }
  }

  .chevron-up {
    path {
      transform: rotate(180deg) translate(-29.5px, -27.5px);
    }
  }

  .chevron-up {
    path {
      transform: rotate(180deg) translate(-29.5px, -27.5px);
    }
  }

  .chevron-left {
    path {
      transform: rotate(90deg) translate(-20.5px, -27.5px);
    }
  }

  .chevron-right {
    path {
      transform: rotate(270deg) translate(-29.5px, -22.5px);
    }
  }

  .remove {
    path {
      transform: rotate(45deg) translate(-20px, -25px);
    }
  }


  .logout {
    transform: rotate(90deg);
  }
}
</style>
